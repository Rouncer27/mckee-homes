...
const MainFloor = ({ data, setSeletedPlans }) => {
  ...
  useEffect(() => {
    setSeletedPlans(prevState => {
      return {
        ...prevState,
        mainFloor: data[activePlan - 1]?.planPdf?.mediaItemUrl,
        mainFloorTitle: data[activePlan - 1]?.planTitle,
      }
    })
  }, [activePlan])
