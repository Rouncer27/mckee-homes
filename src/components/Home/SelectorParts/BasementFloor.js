...
const BasementFloor = ({ data, setSeletedPlans }) => {
  ...
  useEffect(() => {
    setSeletedPlans(prevState => {
      return {
        ...prevState,
        basement: data[activePlan - 1]?.planPdf?.mediaItemUrl,
        basementTitle: data[activePlan - 1]?.planTitle,
      }
    })
  }, [activePlan])
              ...
              return (
                <div key={index}>
                  <input
                    type="radio"
                    name="basementfloor"
                    id={`basementfloor-${index + 1}`}
                    value={index + 1}
                    checked={activePlan === index + 1}
                    onChange={evt => ChangeActivePlan(evt)}
                  />
                  <label htmlFor={`basementfloor-${index + 1}`}>
